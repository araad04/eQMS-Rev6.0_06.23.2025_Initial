  ...
  try {
    const documents = await db
      .select({
        ...
        // Enhanced fields not in base schema - using defaults
        completionPercentage: sql`0`.as('completionPercentage'),
        complianceStatus: sql`'not_started'`.as('complianceStatus'),
        riskClassification: sql`'low'`.as('riskClassification'),
        regulatoryImpact: sql`false`.as('regulatoryImpact'),
